...
namespace {
QString cleanVersionString(QString ver)
{
...
PrismUpdaterApp::PrismUpdaterApp(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    if (!userVersion.isEmpty()) {
        QString cleanUserVer = userVersion;
    ...
    if (!version.isEmpty()) {
        ...
        if (!versionParts.isEmpty()) {
            m_prismVersionBuild = versionParts.takeFirst().toInt();
        } else {
            m_prismVersionBuild = 0;
        }
    }
...
void PrismUpdaterApp::run()
{
    ...
    if (m_prismVersion.isEmpty() && !loadPrismVersionFromExe(m_prismExecutable)) {
        m_prismVersion = BuildConfig.versionString();
        ...
        m_prismVersionBuild = BuildConfig.VERSION_BUILD;
    ...
    qDebug() << "Version build:" << m_prismVersionBuild;
...
QList<GitHubRelease> PrismUpdaterApp::newerReleases()
{
    ...
    for (const auto& rls : nonDraftReleases()) {
        ...
        auto rlsVer = Version(cleanVersionString(rls.tag_name));
        if (rlsVer > current_ver) {
            newer.append(rls);
        }
    }
...
GitHubRelease PrismUpdaterApp::selectRelease()
{
    ...
    SelectReleaseDialog dlg(Version(cleanVersionString(m_prismVersion)), releases);
...
bool PrismUpdaterApp::loadPrismVersionFromExe(const QString& exePath)
{
    ...
    auto version = firstParts.takeLast().trimmed();
    m_prismBinaryName = firstParts.join(' ').trimmed();
    if (version.contains('-')) {
        auto index = version.indexOf('-');
        m_prsimVersionChannel = version.mid(index + 1);
        version = version.left(index);
...
void PrismUpdaterApp::loadReleaseList()
{
    ...
    m_repoOwner = pathParts.takeFirst();
    m_repoName = pathParts.takeFirst();
...
void PrismUpdaterApp::loadReleaseListFromCdn()
{
    ...
    auto cdnUrl = QString("https://raw.githubusercontent.com/%1/%2/develop/releases.json?t=%3")
                      .arg(m_repoOwner, m_repoName, QString::number(QDateTime::currentSecsSinceEpoch()));
    qDebug() << "Fetching release list via CDN cache from" << cdnUrl;

    auto [download, response] = Net::Request::makeByteArray(cdnUrl);
    download->setNetwork(m_network.get());
    m_currentUrl = cdnUrl;
...
void PrismUpdaterApp::loadReleaseListFromApi(int page)
{
    ...
    auto apiUrl = QString("https://api.github.com/repos/%1/%2/releases").arg(m_repoOwner, m_repoName);
    qDebug() << "Fetching release list from GitHub REST API" << apiUrl << "page" << page;
    downloadReleasePage(apiUrl, page);
}
...
void PrismUpdaterApp::downloadReleasePage(const QString& apiUrl, int page)
{
    ...
    githubApiHeaders->addHeaders({
        ...
        { .headerName = "User-Agent", .headerValue = QString("PrismLauncher-Updater/%1").arg(BuildConfig.printableVersionString()).toUtf8() },
    });
    ...
    if (!token.isEmpty()) {
        githubApiHeaders->addHeaders({ { .headerName = "Authorization", .headerValue = QString("Bearer %1").arg(token).toUtf8() } });
    }
    ...
    connect(download.get(), &Net::Request::succeeded, this, [this, response, perPage, apiUrl, page]() {
        ...
        } else if (!(numFound.value() < perPage) && m_selectUI &&
...
void PrismUpdaterApp::loadReleaseListFromAtomFeed()
{
    ...
    auto atomUrl = QString("https://github.com/%1/%2/releases.atom").arg(m_repoOwner, m_repoName);
    qDebug() << "Fetching release list via GitHub Atom feed from" << atomUrl;

    auto [download, response] = Net::Request::makeByteArray(atomUrl);
    download->setNetwork(m_network.get());
    m_currentUrl = atomUrl;
...
Result<int> PrismUpdaterApp::parseReleasePage(const QByteArray* response)
{
    ...
    for (auto releaseJson : doc) {
        ...
        QString cleanTag = cleanVersionString(release.tag_name);
        release.version = Version(cleanTag);
...
GitHubRelease PrismUpdaterApp::getLatestRelease()
{
    ...
    for (const auto& release : m_releases) {
        ...
        auto relVer = Version(cleanVersionString(release.tag_name));
        auto latestVer = latest.isValid() ? Version(cleanVersionString(latest.tag_name)) : Version();
        if (!latest.isValid() || (relVer > latestVer)) {
            latest = release;
        }
    }
    return latest;
...
bool PrismUpdaterApp::needUpdate(const GitHubRelease& release) const
{
    ...
    auto currentVer = Version(cleanVersionString(localVer));
    auto releaseVer = Version(cleanVersionString(release.tag_name));
    return currentVer < releaseVer;
}
